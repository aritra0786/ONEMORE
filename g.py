...
TOKEN = '6303534622:AAFt88m2Wqdqai-hcZgF4qgx6FdtP8ZgHA4'
...
FORWARD_CHANNEL_ID = -1002167122834
CHANNEL_ID = -1002167122834
error_channel_id = -1002167122834
...
REMOTE_HOST = None  # Initialize as None or an empty string
IP_CHECK_INTERVAL = 60  # Time in seconds to check IP address
...
async def update_remote_host():
    ...
    if current_ip and current_ip != REMOTE_HOST:
        REMOTE_HOST = current_ip
        logging.info(f"Updated REMOTE_HOST to: {REMOTE_HOST}")

async def ip_monitor_loop():
    while True:
        await update_remote_host()
...
async def start_asyncio_tasks():
    asyncio.create_task(ip_monitor_loop())
...
def attack_command(message):
    ...
    try:
        ...
        bot.register_next_step_handler(message, lambda msg: asyncio.run(process_attack_command(msg)))
    except Exception as e:
...
def main():
    ...
    loop.run_until_complete(start_asyncio_tasks())
    start_bot()
